...
@section('title', 'Tambah Layanan Tambahan Baru')

@section('content')
<div class="min-h-screen py-12 px-4 mt-20">
    <div class="max-w-2xl mx-auto">
        <div class="bg-white rounded-xl shadow-2xl p-8 sm:p-10">
            <h1 class="text-3xl font-bold text-gray-800 mb-6 border-b pb-4">Tambah Add-on Baru</h1>
            
            <form action="{{ route('addons.store') }}" method="POST" class="space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name') }}" 
                           ...
                           placeholder="Contoh: Sarapan Buffet, Ekstra Bed" required>
                    @error('name')
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3" 
                              class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500" 
                              placeholder="Penjelasan singkat mengenai layanan ini...">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex justify-end space-x-4 pt-4">
                    ...
                    <button type="submit" 
                            class="bg-indigo-600 text-white px-6 py-2.5 rounded-lg shadow-md hover:bg-indigo-700 transition duration-150 font-semibold">
                        Simpan Add-on
                    </button>
                </div>
            </form>
